            ...
            <tbody>
              {data.sentences.map((char,index) => (
                <tr key={`id-${index}`} onClick={() => audio(char.sentence)} className='cursor-pointer'>
                  <td className="w-fit border-2 dark:border-gray-700 p-2 sm:p-4 text-xl sm:text-2xl md:text-3xl lg:text-3xl text-center scale-150 md:scale-100">
                    {index + 1}
                  </td>
                  ...
                  <td className="w-1/2 border-2 dark:border-gray-700 p-2 sm:p-4 md:text-xl">{char.pronunciation}</td>
                </tr>
              ))}
            </tbody>
          </table>
